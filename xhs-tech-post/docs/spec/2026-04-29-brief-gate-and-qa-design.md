# 规格:简报关卡 + 发布前自检(xhs-tech-post v1.1)
...
- **④ 简报关卡(Brief Gate)** — 把 Step 1 从"问 2-3 个问题"形式化为 6 字段 schema,带拒绝协议(缺 `evidence` 或 `trade_off` 时不允许往下走)。
- **③ 发布前自检(Pre-flight QA)** — 在 Step 4 与 Step 5 之间插入 Step 4.5,model 在交付 HTML 前必须 emit 一个逐卡自审表(字符数、specifics、手绘元素、trade-off 是否存在)。任何 FAIL 自动重生 + 复检。
